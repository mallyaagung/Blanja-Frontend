...
export const updateProduct = (data, id ,saveImage,setShow) => async (dispatch) => {
  try {
    ...
    const products = await axios.put(process.env.REACT_APP_BACKEND + "/product/" + id, formData ,{
      headers: {
...
export const createProduct = (data, saveImage, setShow) => async (dispatch) => {
  try {
    ...
    formData.append("description", data.description);
    formData.append("photo", saveImage);
    const products = await axios.post(process.env.REACT_APP_BACKEND + "/product", formData ,{
      headers: {
          "Content-Type": "multipart/form-data",
      },
  });
